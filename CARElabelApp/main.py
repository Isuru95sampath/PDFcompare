...
def main():
    ...
    with col2:
        ...
        # Fix 1: Add proper conditional logic with a button
        if merged_po_file and st.button("Extract PO Details", key="extract_po"):
            # Fix 2: Add file validation
        ...
        if wo_file:
            ...
            for po in st.session_state.po_data:
                for item in po['items']:
                    size = item['size']
                    # Fix 3: Handle potential None or empty values
                    quantity = 0
                    if item['quantity'] and item['quantity'] != '':
                        try:
                            quantity = float(item['quantity'])
                        except (ValueError, TypeError):
                            st.warning(f"Invalid quantity value: {item['quantity']}")
                            quantity = 0
